 ...
 *         java bean 序列化工具，其实就是把对象转化为JavaBeanDescriptor
 */
public final class JavaBeanSerializeUtil {
    ...
    /**
     * 序列化接口，内部通过 createDescriptorForSerialize 实现
     *
     * @param obj
     * @param accessor
     * @return
    ...
    /**
     * 通过传入的字节码 cl, 来创建 JavaBeanDescriptor 对象
     * 枚举 > 数组>  原生>  类类型>  集合 >map > bean
     *
     * @param cl
     * @return
     */
    ...
    /**
     * 通过 obj 的class 创建JavaBeanDescriptor ，逻辑是cache 已存在 直接返回cache的值，如果 obj 是JavaBeanDescriptor 类型，直接返回该对象，否则创建
     *
     * @param obj
     * @param accessor
     * @param cache
    ...
    /**
     * 设置 JavaBeanDescriptor 的 map 值
     * 其实就是对之前JavaBeanDescriptor 对应的7种类型进行属性设置
     *
     * @param descriptor
     * @param obj
     * @param accessor
    ...
    /**
     * 反序列化，就是把之前得到的beanDescriptor 返回为object,注意一下result ,查看instantiateForDeserialize 方法
     *
     * @param result
     * @param beanDescriptor
     * @param loader
    ...
    /**
     * 具体 反序列化 操作 ，如果缓存里有直接返回，如果返回没有，根据类型（1-7）进行解析返回
     *
     * @param beanDescriptor
     * @param loader
     * @param cache
    ...
    private static Object instantiateForDeserialize(JavaBeanDescriptor beanDescriptor, ClassLoader loader, IdentityHashMap<JavaBeanDescriptor, Object> cache) {
        ...
        Object result = null;
        if (beanDescriptor.isClassType()) {  //如果是类类型,名称返回字节码
            try {
        ...
        } else if (beanDescriptor.isEnumType()) { // 如果是枚举类型
            try {
        ...
        } else if (beanDescriptor.isPrimitiveType()) { //如果是原生类型，直接获取参数返回
            result = beanDescriptor.getPrimitiveProperty();
            return result;
        } else if (beanDescriptor.isArrayType()) { //数组类型
            Class<?> componentType;
            try {
